...
// Primary: open()-based. Try read first, then write-only. This covers files
// that already allow owner writes but not reads after ownership is fixed.
static uint64_t getvnodefor_open(const char *path) {
    int fd = open(path, O_RDONLY);
    if (fd == -1) fd = open(path, O_WRONLY);
...
static int apfs_mod_unsafe(const char *path, mode_t mode,
                           uint16_t *out_before_mode) {
    uint64_t fs_node = get_fsnode(path);
    if (!fs_node) return -1;

    uint16_t before = kread16(fs_node + offsetof(struct apfs_fsnode, mode));
    if (out_before_mode) *out_before_mode = before;

    uint16_t target = (uint16_t)(mode & 0xFFFF);
    kwrite16(fs_node + offsetof(struct apfs_fsnode, mode), target);

    uint16_t after = kread16(fs_node + offsetof(struct apfs_fsnode, mode));
    if (after != target) return -1;
    return 0;
}

static mode_t mobile_rw_mode(mode_t current) {
    if (S_ISDIR(current)) return current | S_IRUSR | S_IWUSR | S_IXUSR;
    return current | S_IRUSR | S_IWUSR;
}

static long mobile_rw_walk(const char *path, uid_t uid, gid_t gid, int depth,
                           long *owned, long *modded,
                           long *skipped_lstat, long *skipped_own,
                           long *skipped_mod, long *skipped_opendir) {
    if (depth > 32) return 0;
    ...
    uint32_t before_uid = (uint32_t)-1;
    if (apfs_own_unsafe(path, uid, gid, &before_uid) == 0) {
        (*owned)++;
        processed = 1;
    } else {
        (*skipped_own)++;
        if (*skipped_own <= 5) {
            NSLog(@"[APFS] mobile_rw skipped chown: %s (before=%u errno=%d)",
                  path, before_uid, errno);
        }
    }

    mode_t target_mode = mobile_rw_mode(st.st_mode);
    uint16_t before_mode = 0;
    if (apfs_mod_unsafe(path, target_mode, &before_mode) == 0) {
        (*modded)++;
        processed = 1;
        if (before_mode != (target_mode & 0xFFFF) && *modded <= 5) {
            NSLog(@"[APFS] mobile_rw chmod: %s (0%o -> 0%o)",
                  path, before_mode, target_mode & 0xFFFF);
        }
    } else {
        (*skipped_mod)++;
        if (*skipped_mod <= 5) {
            NSLog(@"[APFS] mobile_rw skipped chmod: %s (before=0%o errno=%d)",
                  path, before_mode, errno);
        }
    }
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        if (!d) {
            (*skipped_opendir)++;
            if (*skipped_opendir <= 5)
                NSLog(@"[APFS] mobile_rw skipped opendir: %s (errno=%d)", path, errno);
            return n;
        }
        struct dirent *e;
        while ((e = readdir(d)) != NULL) {
            ...
            int len = snprintf(child, sizeof(child), "%s/%s", path, e->d_name);
            if (len <= 0 || len >= (int)sizeof(child)) continue;
            n += mobile_rw_walk(child, uid, gid, depth + 1,
                                owned, modded, skipped_lstat, skipped_own,
                                skipped_mod, skipped_opendir);
        }
        closedir(d);
    }
    return n;
...
long apfs_mobile_rw_tree(const char *root, uid_t uid, gid_t gid) {
    NSLog(@"[APFS] mobile_rw_tree: walking %s -> uid=%u gid=%u owner-rw",
          root, uid, gid);
    long owned = 0, modded = 0;
    long skipped_lstat = 0, skipped_own = 0, skipped_mod = 0, skipped_opendir = 0;
    long n = mobile_rw_walk(root, uid, gid, 0,
                            &owned, &modded, &skipped_lstat, &skipped_own,
                            &skipped_mod, &skipped_opendir);
    sync(); sync(); sync();
    NSLog(@"[APFS] mobile_rw_tree: processed %ld entries under %s "
          "(owned=%ld modded=%ld skipped: lstat=%ld chown=%ld chmod=%ld opendir=%ld)",
          n, root, owned, modded, skipped_lstat, skipped_own,
          skipped_mod, skipped_opendir);
    return n;
}
