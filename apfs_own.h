...
// Recursively chown every file/dir under `root` to (uid, gid), then make
// owner read/write stick across the tree. Directories also get owner execute
// so Filza can traverse them and save edits inside. Returns number of entries
// that were successfully processed by either ownership or mode writes.
